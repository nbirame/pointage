...
class RapportWizard(models.TransientModel):
    ...
    def _compute_total_number_of_working_hours(self):
        ...
        for employee in employees:
            ...
            # self.total_number_of_working_hours = int((self.nombre_jours_sans_weekend(self.date_in_get_rapport, self.date_end_get_rapport) - number_of_days_absence_legal) * heure_travail.worked_hours)
            if equipe_mission:
                ...
                for agent in equipe_mission:
                    if (agent.mission_id.state == "en_cours" or agent.mission_id.state == "terminer") and ((agent.mission_id.date_depart >= self.date_in_get_rapport and agent.mission_id.date_retour <= self.date_end_get_rapport) or (agent.mission_id.date_depart <= self.date_in_get_rapport and agent.mission_id.date_retour <= self.date_end_get_rapport) or (agent.mission_id.date_depart <= self.date_in_get_rapport and agent.mission_id.date_retour >= self.date_end_get_rapport)) and (
                        ...
                        print(f"Nombre de jours en mission {number_day_of_mission} du {agent.mission_id.date_depart} au {agent.mission_id.date_retour}")
                        print(f"Nombre de jours en conge {absence_days_hollidays}")
                        print(f"Nombre de jours en ferier {number_day_of_party}")
                        number_of_days_absence_legal = absence_days_hollidays + number_day_of_party + number_day_of_mission
                        ...
                        print(f"Nombre de jours a travailler {self.total_number_of_working_hours}")
            else:
                number_of_days_absence_legal = absence_days_hollidays + number_day_of_party
                self.total_number_of_working_hours = int((self.nombre_jours_sans_weekend(self.date_in_get_rapport,
                                                                                    self.date_end_get_rapport) - number_of_days_absence_legal) * heure_travail.worked_hours)
                print(f"Nombre de jours a travailler dans else{self.total_number_of_working_hours}")
